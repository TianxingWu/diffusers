...
class StableDiffusionXLFreeInitPipeline(StableDiffusionXLPipeline):
    model_cpu_offload_seq = "text_encoder->text_encoder_2->image_encoder->unet->vae"
    _optional_components = [
        "tokenizer",
        "tokenizer_2",
    ...
    def init_filter(self, video_length, height, width, filter_params):
        ...
        # self.freq_filter = get_freq_filter_3d(
        #     filter_shape, 
        #     device=self._execution_device, 
        ...
        # )
        self.freq_filter = get_freq_filter_2d(
            filter_shape, 
            device=self._execution_device, 
            filter_type=filter_params.method,
            n=filter_params.n if filter_params.method=="butterworth" else None,
        ...
        **kwargs,
        ...
        for iter_idx in range(num_iters):
            ...
            if iter_idx == 0:
                ...
                #     current_diffuse_timestep = self.scheduler.config.num_train_timesteps / num_iters * (iter_idx + 1) - 1
                # else:
                #     current_diffuse_timestep = self.scheduler.config.num_train_timesteps - 1
                # current_diffuse_timestep = self.scheduler.config.num_train_timesteps - 1 # diffuse to t=999 noise level
                current_diffuse_timestep = 999 # could be 981? according to inference timestep
                ...
                # sigma = 
                # z_T = latents.to(device) + initial_noise.to(device) * sigma

                # 2. create random noise z_rand for high-frequency
                # z_rand = torch.randn((batch_size * num_images_per_prompt, num_channels_latents, 1, height // self.vae_scale_factor, width // self.vae_scale_factor), device=device)
                z_rand = self.prepare_latents(
                    batch_size * num_images_per_prompt,
                    num_channels_latents,
                    height,
                    width,
            ...
            if use_fast_sampling:
                current_num_inference_steps= int(num_inference_steps / num_iters * (iter_idx + 1))
            ...
            # self.scheduler._step_index = None # debug: reinit step index
            ...
            if has_latents_mean and has_latents_std:
                ...
                latents_output = latents * latents_std / self.vae.config.scaling_factor + latents_mean
            else:
                latents_output = latents / self.vae.config.scaling_factor

            image = self.vae.decode(latents_output, return_dict=False)[0]
            ...
            # save img
            save_path_current = os.path.join(save_dir, f"iter_{iter_idx}.png")
            image[0].save(save_path_current)
